...
static usbd_device *stm32l0x2_usbd_init(void);
static void stm32l0x2_set_address(usbd_device *usbd_dev, uint8_t addr);
static void stm32l0x2_ep_setup(usbd_device *usbd_dev, uint8_t addr,
			       uint8_t type, uint16_t max_size,
			       void (*callback) (usbd_device *usbd_dev,
						 uint8_t ep));
static void stm32l0x2_endpoints_reset(usbd_device *usbd_dev);
static void stm32l0x2_ep_stall_set(usbd_device *usbd_dev, uint8_t addr,
				   uint8_t stall);
static uint8_t stm32l0x2_ep_stall_get(usbd_device *usbd_dev, uint8_t addr);
static void stm32l0x2_ep_nak_set(usbd_device *usbd_dev, uint8_t addr,
				 uint8_t nak);
static uint16_t stm32l0x2_ep_write_packet(usbd_device *usbd_dev, uint8_t addr,
					  const void *buf, uint16_t len);
static uint16_t stm32l0x2_ep_read_packet(usbd_device *usbd_dev, uint8_t addr,
					 void *buf, uint16_t len);
static void stm32l0x2_poll(usbd_device *usbd_dev);

static uint8_t force_nak[8];
static struct _usbd_device usbd_dev;

const struct _usbd_driver stm32l0x2_usb_driver = {
	.init = stm32l0x2_usbd_init,
	.set_address = stm32l0x2_set_address,
	.ep_setup = stm32l0x2_ep_setup,
	.ep_reset = stm32l0x2_endpoints_reset,
	.ep_stall_set = stm32l0x2_ep_stall_set,
	.ep_stall_get = stm32l0x2_ep_stall_get,
	.ep_nak_set = stm32l0x2_ep_nak_set,
	.ep_write_packet = stm32l0x2_ep_write_packet,
	.ep_read_packet = stm32l0x2_ep_read_packet,
	.poll = stm32l0x2_poll,
};
...
static void stm32l0x2_set_address(usbd_device *dev, uint8_t addr)
{
	(void)dev;
...
static void stm32l0x2_ep_setup(usbd_device *dev, uint8_t addr, uint8_t type,
			       uint16_t max_size,
			       void (*callback) (usbd_device *usbd_dev,
						 uint8_t ep))
{
...
static void stm32l0x2_endpoints_reset(usbd_device *dev)
{
	int i;
...
static void stm32l0x2_ep_stall_set(usbd_device *dev, uint8_t addr,
				   uint8_t stall)
{
	(void)dev;
	if (addr == 0) {
...
static uint8_t stm32l0x2_ep_stall_get(usbd_device *dev, uint8_t addr)
{
	(void)dev;
	if (addr & 0x80) {
...
static void stm32l0x2_ep_nak_set(usbd_device *dev, uint8_t addr, uint8_t nak)
{
	(void)dev;
...
static uint16_t stm32l0x2_ep_write_packet(usbd_device *dev, uint8_t addr,
				     const void *buf, uint16_t len)
{
	(void)dev;
	addr &= 0x7F;
...
static uint16_t stm32l0x2_ep_read_packet(usbd_device *dev, uint8_t addr,
					 void *buf, uint16_t len)
{
	(void)dev;
...
static void stm32l0x2_poll(usbd_device *dev)
{
